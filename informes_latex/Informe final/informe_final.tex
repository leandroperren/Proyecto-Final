...
Observando estos T0, se puede ver que los mismos tienen valores inferiores a los 3 segundos, por lo tanto estos valores caen fuera del rango [3.50 - 4.70 s] utilizado por el algoritmo para etiquetarlos como segmentos con ronquidos, de todas maneras representan una minoría.

En el Cuadro \ref{tabla:falsos_positivos_matlab}, se muestran algunos de los falsos positivos que se detectaron con el algoritmo, correspondientes a los pacientes 1, 6, 8 y 9. Como se puede apreciar, presentan valores de T0 similares a los segmentos con ronquidos. De los análisis puntuales realizados, fue notorio que las señales EDF en esos segmentos sin estar etiquetados como ronquidos presentan algunos picos aislados, esto se ve en algunas señales en mayor proporción que en otras, como el caso del paciente 9. Debido a esto el algoritmo los detecta como ronquidos. Igual que en el punto anterior y debido a que es un fenómeno presentado en algunas señales se tendrá en cuenta en la aplicación final Android, ya que a la hora de capturar las muestras desde un dispositivo estos ``picos'' aislados no deberían aparecer.
...
Para el desarrollo de la aplicación android se seguirán los principios de la ingeniería de software, usando como referencia el libro ``Ingeniería de Software, Ian Sommerville'', tomando en cuenta un modelo de desarrollo en cascada y se seguirán las etapas mencionadas a continuación:
...
\begin{table}[htbp]
	\begin{center}
		\begin{tabular}{|l|}
			\hline
			\textbf{CASO DE USO: Iniciando captura} \\
			\hline
...
\chapter*{Anexo A}
\thispagestyle{empty}
\addcontentsline{toc}{chapter}{Anexo A} % esto añade el capitulo en el índice
